...
__kernel __attribute__((reqd_work_group_size(OUT_KWG, OUT_BWG, 1)))
void out_transform(__global const net_t * restrict M,
                                     __global net_t * restrict Y,
                                     const int K,
                                     const int Kpad, const int Ppad,
                                     const int batch_size,
...
__kernel
void fused_bn_res_scale(__global net_t * restrict input,
                        __global net_t * restrict output,
                        __global const net_t * restrict residual,
                        __constant const net_t * restrict s1,
                        __constant const net_t * restrict s2) {
    const int ch_size = BOARD_W * BOARD_H;
    const int num_channels = get_local_size(0);
    const int channel = get_local_id(0);
    const int batch = get_group_id(0);

    // max channels
    __local float avg[1024];
    __local float mid[128];

    float r = 0.0f;

#pragma unroll
    for(int i=0; i<ch_size; i++) {
        float rr = vload_net_t(batch * num_channels * ch_size + channel * ch_size + i, input);
        r = r + rr;
    }
    ...
    if(channel < num_channels/8) {
        r = 0.0f;
        for(int i=0; i<num_channels; i++) {
            r = r + vload_net_t(channel * num_channels + i, s1) * avg[i];
        }
        mid[channel] = r > 0.0f ? r : 0.0f;
    }

    barrier(CLK_LOCAL_MEM_FENCE);

    r = 0.0f;
    for(int i=0; i<num_channels/8; i++) {
        r = r + vload_net_t(channel * (num_channels/8) + i, s2) * mid[i];
    }
    r = 1.0f / (1.0f + exp(-r));
#pragma unroll
    for(int i=0; i<ch_size; i++) {
        int offset = batch * num_channels * ch_size + channel * ch_size + i;
        float res = vload_net_t(offset, residual);
        float rr = vload_net_t(offset, input);
