...
static bool cfg_sgemm_exhaustive = false;
...
const auto TUNER_FILE_LOCAL = std::string("gmgm_opencl_tuning");

const std::string gmgm_file(std::string file) {
#ifdef _WIN32
...
std::string Tuner<net_t>::tune_sgemm(const int m, const int n, const int k,
    ...
    for (auto & p : valid_params) {
        ...
        if (error < getTunerMaxError<net_t>() && (best_time == 0 || sum < best_time)) {
            ...
            myprintf("(%lu/%lu) %s %.4f ms (%.1f GFLOPS)\n",
    ...
    if (best_time == 0) {
        if (failed_compile > 0) {
            myprintf("Failed to compile: %d kernels.\n", failed_compile);
        }
        if (failed_enqueue > 0) {
            myprintf("Failed to enqueue: %d kernels\n", failed_enqueue);
        }
        if (failed_error > 0) {
            myprintf("Too high error: %d kernels\n", failed_error);
        }
        myprintf("Failed to find a working configuration.\nCheck your OpenCL drivers.\n");
        myprintf("Minimum error: %f. Error bound: %f\n", min_error, getTunerMaxError<net_t>());
...
void Tuner<net_t>::store_sgemm_tuners(const int m, const int n, const int k,
    ...
    auto tuner_file = gmgm_file(TUNER_FILE_LOCAL);
