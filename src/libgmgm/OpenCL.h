...
private:
    ...
    bool is_final_conv{false};
    bool has_squeeze_layer{false};
...
private:
    ...
    cl::Kernel m_fused_bn_res_scale_kernel;
    cl::Buffer m_inBuffer;
    cl::Buffer m_tempBuffer;
    cl::Buffer m_inBuffer2;
    cl::Buffer m_VBuffer;
    cl::Buffer m_MBuffer;
    cl::Buffer m_midBuffer_pol;
    cl::Buffer m_midBuffer_val;
...
public:
    ...
    void push_residual(unsigned int filter_size,
                       ...
                       const std::vector<net_t>& sq_1,
                       const std::vector<net_t>& sq_2) {
        size_t layer = get_layer_count();
        ...
        m_layers[layer].filter_size = filter_size;
        m_layers[layer].channels = channels;
        m_layers[layer].has_squeeze_layer = (sq_1.size() > 0);

        if(m_layers[layer].has_squeeze_layer) {
            push_weights(layer, sq_1);
            push_weights(layer, sq_2);
        }
    }
